...
import 界面.上下文.bsfunction as bs
...
import 文件.bpformat.function_bpnumpy as by
import 文件.bpformat.function_bpnumpy_inverted as byed
...
class 类:
    def __init__(self, frametree, menu):
        ...
        menu.add_cascade(label = "Checkpoints/cpkt", menu=菜单)
        
        菜单.add_command(label="文件分块", command=self.文件分块)
        菜单.add_command(label="PK66分块", command=self.PK66分块)
