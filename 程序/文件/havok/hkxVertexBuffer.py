...
class hkxVertexBufferVertexData:
    def __init__(self, bp, hkafile):
        ...
        self.vectorData = [readhavokuint(bp, typedata) for i in range(numdata)]
        numdata, typedata = readhavokint(bp), bp.readuint8()
        self.floatData = [readhavokuint(bp, typedata) for i in range(numdata)]
        numdata, typedata = readhavokint(bp), bp.readuint8()
        self.uint32Data = [readhavokuint(bp, typedata) for i in range(numdata)]
