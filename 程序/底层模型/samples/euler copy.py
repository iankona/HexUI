...
class EULERSampler:
    ...
    def p_sample(self, tf, X_t, cond, ts, unconditional_guidance_scale, unconditional_conditioning):
        ...
        sigma = self.sigmas[ts] # ts == tensor([951], device='cuda:0')
        # t = self.sigma_to_t(sigma) # tensor([951.0], device='cuda:0')
        # t = torch.full((batch_size,), timestep, device=device, dtype=torch.long)
        e_t = self.model.apply_model(X_t*(1.0/sigma), ts, cond)
        model_out = e_t 

        # e_t_uncond = self.model.apply_model((1.0/sigma)*X_t, t, unconditional_conditioning)
        # e_t = self.model.apply_model((1.0/sigma)*X_t, t, cond)
        # e_t = e_t_uncond + unconditional_guidance_scale * (e_t - e_t_uncond)
        # model_out = e_t 


        # denoised =  X_t + (-sigma) * model_out


        # d = (X_t - denoised) / (sigma*s_in)
        d = -model_out
